...
router.get('/kakao/callback',
  ...
  async (req, res) => {
    try {
      ...
      res.redirect(`/login-success.html?token=${token}&refreshToken=${refreshToken}&provider=kakao&role=${req.user.role}&service=${serviceType}`);
...
router.get('/naver/callback',
  ...
  async (req, res) => {
    try {
      ...
      // 토큰을 URL 파라미터로 전달하고 리다이렉트
      const serviceType = req.user.service_type || 'elderly_protection';
      res.redirect(`/login-success.html?token=${token}&refreshToken=${refreshToken}&provider=naver&role=${req.user.role}&service=${serviceType}`);
...
router.get('/google/callback',
  ...
  async (req, res) => {
    try {
      ...
      // 토큰을 URL 파라미터로 전달하고 리다이렉트
      const serviceType = req.user.service_type || 'elderly_protection';
      res.redirect(`/login-success.html?token=${token}&refreshToken=${refreshToken}&provider=google&role=${req.user.role}&service=${serviceType}`);
